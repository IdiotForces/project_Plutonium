...
#define MR_DIVISOR ((double) 4294967291U)/* File:     pth_ll_rwl.c
 *
...
#  endif
   ...
   thread_handles = reinterpret_cast<pthread_t *>(malloc(thread_count*sizeof(pthread_t)));
...
int Insert(int value) {
   ...
   if (curr == NULL || curr->data > value) {
      temp = reinterpret_cast<list_node_s *>(malloc(sizeof(struct list_node_s)));
      temp->data = value;
      temp->next = curr;
      if (pred == NULL)
         head = temp;
